Ext.define('ExFrm.view.ide.TmplPartListController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.tmplpartlist',
	init:function(){
		this.onSearch();
	},
    onSearch:function(){
		...
		this.page= 1;
		this.totCount = 0;
        console.log('여기',this.getViewModel().getStore('searchInfo'));
    ...
    onDblClickMain:function(dv, record, item, index, e){
    	...
    	this.lookupReference('tmplImg').setHtml('<img width="100%" src= "./lib/tmplimg/' + 
    		fileName + '.png">');  
		...
		Ext.Ajax.request({
			...
			url: lboServerPath + 
				...
				lboFileSeperator + fileName + '.link',
		...
		Ext.Ajax.request({
			...
			url: lboServerPath + 
				lboFileSeperator + 'lib' + 
				lboFileSeperator + 'tmpljs' + 
				lboFileSeperator + fileName +'_view.js',
			success:function(res){
		...
		Ext.Ajax.request({
			...
			url: lboServerPath + 
				lboFileSeperator + 'lib' + 
				lboFileSeperator + 'tmpljs' + 
				lboFileSeperator + fileName + '_controller.js',
			success:function(res){
		...
		Ext.Ajax.request({
			...
			url: lboServerPath + 
				lboFileSeperator + 'lib' + 
				lboFileSeperator + 'tmpljs' + 
				lboFileSeperator + fileName +'_model.js',
			success:function(res){
