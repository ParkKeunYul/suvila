Ext.define('ExFrm.view.rec.rec020w_04Controller', {
    extend: 'ExFrm.view.base.ViewController',
    alias: 'controller.rec020w_04',
    ...
    dsclassMgtCallback : function(me, success, form, action){
    	setTimeout(function(){
    		me.callStore(me, 'ds_templeStayKind', '', null ,null);
    	},50);
    },     
    ...
    onSelect : function(){
    	...
    	var V_TEMPLE_STAY_CD = me.lookupReference('lc_KindInfo').getExValue();
    	var V_FDATE          = "";
    	
    	console.log('V_TEMPLE_STAY_CD = ', V_TEMPLE_STAY_CD);
    	if(V_TEMPLE_STAY_CD != null && V_TEMPLE_STAY_CD !="" && V_TEMPLE_STAY_CD != undefined){
    		var kind_record = me.getViewModel().getStore('ds_templeStayKind').findRecord('TEMPLE_STAY_CD', V_TEMPLE_STAY_CD);
    		console.log('kind_record = ', kind_record);
    		V_FDATE = kind_record.get("FDATE");
    	}
    	...
    	var params = {
    		...
    		,V_PROPOSAL_BUD_NO  : me.lookupReference('hid_bud_no').getExValue()
       		...
       		,V_TEMPLE_STAY_CD   : me.lookupReference('lc_KindInfo').getExValue()
       		,V_FDATE            : V_FDATE
    ...
    onSelectCallback : function(me, success, form, action){
    	...
    	if(success){
    		me.lookupReference('rec020w_04_a').getView().select(0);    		
    	}
    },         
    onExcel : function(){
    	var me = this;    
    	var grid = me.lookupReference('rec020w_04_a');
    	exCommon.excelDown(grid, 'temple', '템플스테이 접수내역',  me.getViewModel().getStore('ds_detail').getCount());    	
    },
    onExcelMisu : function(){
    	var me     = this;    
    	var grid   = me.lookupReference('rec020w_04_b');
    	var record = me.lookupReference('rec020w_04_a').getView().getSelectionModel().getSelection()[0];
    	
    	exCommon.excelDown(grid, 'temple', '템플스테이 납부내역('+record.get("PROPOSAL_BUD_NM")+")");    	
    },
    ...
    dsMisuCallback : function(me, success, form, action){
    	if(success){
    		me.lookupReference('rec020w_04_b').getView().select(0);
    	}
    },
