Ext.define('ExFrm.view.rec.rec003w_33Controller', {
    extend: 'ExFrm.view.base.ViewController',
    alias: 'controller.rec003w_33',
    ...
    bokwiCheckChange : function (comp, rowIndex, checked, eOpts ){
    	...
    	for(var i = 0; i < cnt ; i++){    		
    		if(rowIndex != i){
    			me.getViewModel().getStore('ds_giJaeBokwi').getAt(i).set("MAIN_BOKWI", false);    			
    		}else{
    			me.getViewModel().getStore('ds_giJaeBokwi').getAt(i).set("MAIN_BOKWI", true);
    		}    		
    ...
    onSelect : function(){
    	...
    	setTimeout(function(){
    		me.callStore(me, 'ds_giJae', '', params ,me.onSelectCallback);
    	},50);    	
    },
    onSelectCallback : function(me, success, form, action){
    	console.log('onSelectCallback');
    	if(success){
    		me.lookupReference('rec003w_33_a').getView().select(0);
    ...
    onSelectionChange : function( me2 , record , selections , eOpts ) {
    	...
    	try{
    		...
    		setTimeout(function(){
        		me.callStore(me, 'ds_giJaeSpirit', '', params ,me.dsGijaeCallback);
        	},50);
    ...
    dsGijaeCallback : function(me, success, form, action){
    	if(success){
    		setTimeout(function(){
        		me.callStore(me, 'ds_giJaeBokwi', '', action._params ,null);
        	},50);
    	}
    },    
    dsSaguBokwiCallback : function(me, success, form, action){
    	me.lookupReference('rec003w_33_d').getView().select(0);
    },
    ...
    onSaveKind : function(){
    	...
    	exCommon.fnGridSaveAll(
    			...
    			,'uptData'
    			,'delData'
    			,'/rec/REC003W_33/saveSaguJaeKind.suvila'
    ...
    onSaveYoungga : function(){
    	...
    	exCommon.fnGridSaveAll(
    			...
    			,'uptData'
    			,'delData'
    			,'/rec/REC003W_33/saveYoungga.suvila'
    ...
    onSave : function(){
    	...
    	var cnt = exCommon.ChangeCount('ds_giJae', me);
    	if(cnt == 0){
    	...
    	exCommon.fnGridSaveAll(
    		...
    		,'uptData'
    		,'delData'
    		,'/rec/REC003W_33/uptGijeJesaDate.suvila'
    		,me.onSaveCallback
    	);
    },
    onSaveCallback : function(me, success, form, action){
    	console.log('onSaveBokCallback = ', success);
    	exCommon.fnGridSaveCallback( me, success , action , 'ds_giJae' );    	
    },
    	...
    	if(me.inFnValidate(me)){
        	...
        	Ext.MessageBox.confirm('알림', '저장 하시겠습니까?', function(btn){
        		if (btn == 'yes') {
        			...
        			setTimeout(function(){
        				me.callForm(me, '/rec/REC003W_33/saveBokwi.suvila', me.onSaveBokCallback , false);
        			},50);	
        		}
        	});
    ...
    onAddBok : function (){
    	...
    	var selectedRecord = me.lookupReference('rec003w_33_a').getView().getSelectionModel().getSelection()[0];
    	...
    	me.lookupReference('rec003w_33_d').getView().select(bokCnt);
    },
    oBokUp : function(){
    	var me = this;
    	...
    	var selection = me.lookupReference('rec003w_33_d').getView().getSelectionModel().getSelection()[0];
    	var _idx      = me.lookupReference('rec003w_33_d').getStore().indexOf(selection);
    	
    	if(_idx == 0){
			return false;
		}
    	
    	var upRecord    = me.lookupReference('rec003w_33_d').getStore().getAt(_idx-1);
    ...
    oBokDown : function(){
    	...
    	var downRecord   = me.lookupReference('rec003w_33_d').getStore().getAt(_idx+1);
    ...
    onExcel : function(){
    	var me = this;
    	var grid = me.lookupReference('rec003w_33_a');
    	exCommon.excelDown(grid, 'gijae', '기재',  me.getViewModel().getStore('ds_giJae').getCount());
    },
    ...
    onPrintBok : function(){
    	...
    	if(me.getViewModel().getStore('ds_giJae').getCount() <= 0){
    	...
    	me.openPopup('ExFrm.view.rec.rec000p_02_6',  selectedRecord.data, null);
    },
