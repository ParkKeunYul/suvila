/*기제 천도제 로직이 같아서 기제꺼  동일하게 쓰자*/
Ext.define('ExFrm.view.rec.rec003w_42Controller', {
    extend: 'ExFrm.view.base.ViewController',
    alias: 'controller.rec003w_42',    
    ...
    spritCheckChange : function (comp, rowIndex, checked, eOpts ){
    	...
    	//console.log(' check DEATH_DAY = ', DEATH_DAY);
    ...
    onSave : function(){
    	...
    	if(me.fnValidate(me)){
    		...
    		var rdo_ApprovalGbn = exCommon.getRepVal(me.lookupReference('rdo_ApprovalGbn_r03_42').getValue().rdo_ApprovalGbn_r03_42,1);
        	...
        	me.lookupReference('ds_giJaeBokwi').setExValue('');
        	...
        	Ext.MessageBox.confirm('알림', '저장 하시겠습니까?',function(btn) {
    			if (btn == 'yes') {
    				me.callForm(me,'/rec/REC003W_42/save.suvila',me.onSaveCallback, false);
    			}
    		});
    ...
    fnValidate  : function(me){
    	...
    	for(var i = 0; i < row_kd ; i++){
    		...
    		if( !exCommon.gridFormVal(me , i , 'ds_giJaeSpirit' , 'rec003w_42_b' , "DECE_REL"    , '망관계' ) ){
    			check = false;
				return false;
			}
    	}// row_kd
    	...
    	for(var i = 0; i < row_bk ; i++){
    		...
    		if( !exCommon.gridFormVal(me , i , 'ds_giJaeBokwi' , 'rec003w_42_c' , "BOKWEJA_NM"    , '복위자명' ) ){
    			check = false;
				return false;
			}
    		
    		if( !exCommon.gridFormVal(me , i , 'ds_giJaeBokwi' , 'rec003w_42_c' , "BOKWI_KIBU_GBN"    , '복위/기부' ) ){
    			check = false;
				return false;
			}
    		
    		if( exCommon.getRepVal(record.get("REP_YN"),"")){
    			if( !exCommon.gridFormVal(me , i , 'ds_giJaeBokwi' , 'rec003w_42_c' , "HYO_REL"    , '행관계' ) ){
        			check = false;
    				return false;
    			}
    		}
		...
		if(!exCommon.gridFormVal(me , 0 , 'ds_giJae' , 'rec003w_42_d' , "EVENT_DATE"    , '제사일' ) ){
			return false;
		}
		
		if(!gf_validateDateDS(me , 0 , 'ds_giJae' , 'rec003w_42_d' , "EVENT_DATE"    , '제사일')){
			return false;
		}
		
		if( !exCommon.gridFormVal(me , 0 , 'ds_giJae' , 'rec003w_42_d' , "EVENT_TIME"    , '제사시간' ) ){
			return false;
		}
