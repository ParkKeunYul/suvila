...
@Repository
public class REC003W_32DAO extends DefaultDAO{

	
	public String NAMEPSACE = "REC003w_32DAO.";
	...
	public boolean save( CommonMap param
		...
		try {
			...
			for(int i = 0; i< ds_giJaeList.size(); i++) {
				...
				sqlSession.insert(NAMEPSACE+"INSERT_GIJAEDETAIL", info);
			...
			/*#기제 영가 입력#*/
			System.out.println("ds_giJaeSpiritList.size = "+ ds_giJaeSpiritList.size());
			for(int i = 0; i< ds_giJaeSpiritList.size(); i++) {
				...
				sqlSession.insert(NAMEPSACE+"INSERT_GIJAESPIRIT", info);
			}// for ds_giJaeSpiritList
			...
			for(int i = 0; i< ds_giJaeBokwiList.size(); i++) {
				...
				sqlSession.insert(NAMEPSACE+"INSERT_GIJAEBOKWI", info);
			...
			accRecInfo.put("SUB_ACCEPT_GBN", 6);
