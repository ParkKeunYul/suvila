...
@Repository
public class REC003W_43DAO extends DefaultDAO{

	
	public String NAMEPSACE = "REC003w_43DAO.";
	
	
	
	public List<Map<String, Object>> SELECT_CHONDO(CommonMap param){
		
		Result result = new Result();
		List<Map<String, Object>>  list = result.resultList;
		sqlSession.select(NAMEPSACE+"SELECT_CHONDO",param,result );
		
		return list;
	}
	
	public List<Map<String, Object>> SELECT_CHONDO_DEA(CommonMap param){
		
		Result result = new Result();
		List<Map<String, Object>>  list = result.resultList;
		sqlSession.select(NAMEPSACE+"SELECT_CHONDO_DEA",param,result );
		
		return list;
	}
	
	public List<Map<String, Object>> SELECT_CHONDO_BOK(CommonMap param){
	
		Result result = new Result();
		List<Map<String, Object>>  list = result.resultList;
		sqlSession.select(NAMEPSACE+"SELECT_CHONDO_BOK",param,result );
	...
	public boolean saveYoungga( CommonMap param
		...
		try {
			...
			for(int i = 0; i< uptList.size(); i++) {
				...
				sqlSession.update(NAMEPSACE+"UPDATE_CHONDOJE_YOUNGGA", info);
			}
	...
	public boolean saveBokwi( CommonMap param
		...
		try {
			...
			sqlSession.insert(NAMEPSACE+"DELETE_CHONDOJE_ALL", param);
			...
			for(int i = 0; i< addList.size(); i++) {
				...
				sqlSession.insert(NAMEPSACE+"INSERT_CHONDOJE", info);
			}
			//txManager.rollback(status);
			txManager.commit(status);
		}catch (Exception e) {
	...
	public boolean updateChundo( CommonMap param
			   				  ,List<Map<String, Object>> uptList) {
		...
		try {
			for(int i = 0; i< uptList.size(); i++) {
				...
				sqlSession.update(NAMEPSACE+"UPDATE_CHUNDO_EVENTDATE", info);
